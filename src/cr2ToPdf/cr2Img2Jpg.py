...
def convert_cr2_folder_to_jpg(cr2_folder, output_jpg_path):
    completeCr2Count = getCr2Count(cr2_folder, True)
    ...
    print(f"convert_cr2_folder_to_jpg: {cr2_folder}({completeCr2Count}) to {output_jpg_path}")
    ...
    for root, dirs, files in os.walk(cr2_folder):
        ...
        for cr2_file in files:
            if cr2_file.lower().endswith('.cr2'):
                ...
                converted_count += 1  # Increment the counter for each converted file
                print(f"({converted_count}/{completeCr2Count}). Converted Cr2 from {cr2_file} to {image_path}")
    ...
    result = {
        ...
        "output": f"{converted_count} jpg files converted from {completeCr2Count} cr2 files",
        "msgs": msgs,
        "success": converted_count == completeCr2Count,
